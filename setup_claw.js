...
const VALIDATOR_SCRIPT = [
    ...
    "",
    "    // 3. Check Diagrams",
    "    // Match code blocks handling both LF and CRLF",
    "    const codeBlocks = content.match(/\`\`\`[a-z]*\\s*[\\r\\n]+[\\s\\S]*?\`\`\`/g) || [];",
    "    ",
    "    // Logical Diagram: Must include mermaid keyword",
    "    const mermaidBlocks = codeBlocks.filter(block => block.includes('mermaid'));",
    "    ",
    "    // Physical Diagram: Non-mermaid BUT contains ASCII art characters (+, |, ┌, └)",
...
### 5.2 アルゴリズム・クエリ詳細
> **Note**: SQLやコードだけでなく、**処理フローや計算ロジックを説明する文章・箇条書き**もここに詳細に記述してください。省略厳禁。
...
