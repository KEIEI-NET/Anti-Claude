...
### 🛡️ Quality Guard (品質監視)

v6.0で導入された **`tools/validate_docs.js`** は、AIの成果物を機械的にチェックします。

- 必須セクション（エラーハンドリング、テスト計画など）の欠落
- プレースホルダー（"..." や "TODO"）の残留
- 構成図（Mermaidおよび物理図）の欠落
これらを検知すると**作業完了を却下**します。AIは合格するまでリライトを強いられます。
